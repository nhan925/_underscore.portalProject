...
class AdminNotificationFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val adminNotiViewModel =
            ViewModelProvider(this).get(AdminNotificationViewModel::class.java)

        _binding = FragmentAdminNotificationBinding.inflate(inflater, container, false)
        val root: View = binding.root

        val textView: TextView = binding.textAdminNoti
        adminNotiViewModel.text.observe(viewLifecycleOwner) {
            textView.text = it
        }
        return root
    }
